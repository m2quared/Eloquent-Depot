...
- <a href="#methods">Methods</a>
    - <a href="#m2quaredrepositorycontractsrepositoryinterface">RepositoryInterface</a>
    - <a href="#m2quaredrepositorycontractsrepositorycriteriainterface">RepositoryCriteriaInterface</a>
    - <a href="#m2quaredrepositorycontractscacheableinterface">CacheableInterface</a>
    - <a href="#m2quaredrepositorycontractspresenterinterface">PresenterInterface</a>
    - <a href="#m2quaredrepositorycontractscriteriainterface">CriteriaInterface</a>
...
### M2quared\Repository\Contracts\RepositoryCriteriaInterface
...
### M2quared\Repository\Contracts\CacheableInterface
...
### M2quared\Repository\Contracts\PresenterInterface

- present($data);

### M2quared\Repository\Contracts\Presentable

- setPresenter(PresenterInterface $presenter);
- presenter();

### M2quared\Repository\Contracts\CriteriaInterface

- apply($model, RepositoryInterface $repository);

### M2quared\Repository\Contracts\Transformable
...
```php
use M2quared\Repository\Eloquent\BaseRepository;
use M2quared\Repository\Criteria\RequestCriteria;
...
class PostRepository extends BaseRepository {
    ...
    public function boot(){
        $this->pushCriteria(app('M2quared\Repository\Criteria\RequestCriteria'));
        ...
    }
...
```php
	public function index()
    {
        $this->repository->pushCriteria(app('M2quared\Repository\Criteria\RequestCriteria'));
        $posts = $this->repository->all();
		...
    }
```
...
`http://m2quared.local/users?search=John%20Doe`

or

`http://m2quared.local/users?search=John&searchFields=name:like`

or

`http://m2quared.local/users?search=john@gmail.com&searchFields=email:=`

or

`http://m2quared.local/users?search=name:John Doe;email:john@gmail.com`

or

`http://m2quared.local/users?search=name:John;email:john@gmail.com&searchFields=name:like;email:=`
...
`http://m2quared.local/users?filter=id;name`
...
`http://m2quared.local/users?filter=id;name&orderBy=id&sortedBy=desc`
...
`http://m2quared.local/users?orderBy=posts|title&sortedBy=desc`
...
`http://m2quared.local/users?orderBy=posts:custom_id|posts.title&sortedBy=desc`
...
`http://m2quared.local/users?with=groups`
...
```php
use M2quared\Repository\Eloquent\BaseRepository;
use M2quared\Repository\Contracts\CacheableInterface;
use M2quared\Repository\Traits\CacheableRepository;
...
Requires [m2quared/laravel-validator](https://github.com/m2quared/laravel-validator). `composer require m2quared/laravel-validator`

Easy validation with `m2quared/laravel-validator`

[For more details click here](https://github.com/m2quared/laravel-validator)
...
```php
use \M2quared\Validator\LaravelValidator;
...
Validation is now ready. In case of a failure an exception will be given of the type: *M2quared\Validator\Exceptions\ValidatorException*
...
```php
use M2quared\Repository\Presenter\FractalPresenter;
...
In your model, implement the interface `M2quared\Repository\Contracts\Presentable` and `M2quared\Repository\Traits\PresentableTrait`

```php
namespace App;

use M2quared\Repository\Contracts\Presentable;
use M2quared\Repository\Traits\PresentableTrait;
...
$repository->setPresenter("M2quared\\Repository\\Presenter\\ModelFractalPresenter");
...
`M2quared\Repository\Presenter\ModelFractalPresenter` is a Presenter default for Models implementing Transformable
...
class PostRepository extends BaseRepository {
    ...
    public function presenter()
    {
        return "M2quared\\Repository\\Presenter\\ModelFractalPresenter";
    }
}
```
...
```php
$this->repository->setPresenter("M2quared\\Repository\\Presenter\\ModelFractalPresenter");
```
